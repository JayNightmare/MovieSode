...
async function getMovie(id: string): Promise<(MovieDoc & { episodes?: EpisodeDoc[] }) | null> {
  const res = await fetch(`${process.env.BASE_URL}/api/movies/${encodeURIComponent(id)}`, {
    cache: "no-store",
  });
...
async function getProgress(movieId: string): Promise<ProgressDoc[]> {
  const res = await fetch(`${process.env.BASE_URL}/api/progress?movieId=${movieId}`, {
    cache: "no-store",
  });
  if (!res.ok) return [];
  const data = await res.json();
...
export default async function MoviePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [movie, progress] = await Promise.all([getMovie(movieId), getProgress(movieId)]);
  if (!movie) return notFound();

  const badgeClass =
    statusStyles[movie.status] || "bg-zinc-800/60 text-zinc-200 border-zinc-700";

  const progressMap = progress.reduce<Record<string, number>>((acc, p) => {
  ...
  return (
    <main className="space-y-8">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/5 bg-gradient-to-br from-zinc-950 via-zinc-900 to-black p-6 shadow-[0_25px_70px_-40px_rgba(0,0,0,0.9)]">
        ...
        <div className="relative space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-bold tracking-tight text-white">{movie.title}</h1>
            <p className="text-sm text-zinc-400">{movie.originalFilename}</p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-white/5 bg-zinc-950/70 p-4 text-sm text-zinc-300">
              ...
              <code className="block rounded bg-zinc-900/70 px-2 py-1 text-xs text-zinc-200">
                {movie.uploadPath || "n/a"}
              </code>
            </div>
            <div className="rounded-2xl border border-white/5 bg-zinc-950/70 p-4 text-sm text-zinc-300 md:col-span-2">
              <div className="mb-2 flex items-center gap-2 text-zinc-100">
      ...
      <MovieViewer movieId={movie._id} episodes={episodes} progress={progressMap} />
    </main>
  );
}
