...
export default function NewMoviePage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus("success");
      setMessage("Upload saved. Processing comes next.");
      ...
      <section className="grid gap-6 lg:grid-cols-[2fr,1fr]">
        ...
        >
          <div className="space-y-2">
            <label className="text-sm font-semibold text-zinc-200" htmlFor="title">
              Title
            </label>
            <input
              id="title"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-zinc-200" htmlFor="episodeLengthMin">
          ...
          <div className="space-y-3">
            <label className="text-sm font-semibold text-zinc-200">Movie file</label>
            <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-white/10 bg-zinc-900/50 p-5">
              <div className="flex items-center gap-3 text-sm text-zinc-300">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-cyan-500/10 text-cyan-200 ring-1 ring-cyan-500/20">
                  <Upload className="h-5 w-5" />
                </div>
                <div>
                  <p className="font-semibold text-white">Select a file</p>
                  <p className="text-xs text-zinc-400">MP4/MKV etc. stored locally, never uploaded.</p>
                </div>
              </div>
              <input
                name="file"
                type="file"
                accept="video/*"
                className="text-sm text-zinc-300 file:mr-3 file:cursor-pointer file:rounded-lg file:border file:border-white/10 file:bg-white/10 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-zinc-100 hover:file:border-white/30"
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-zinc-100 px-4 py-2 text-sm font-semibold text-zinc-900 transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70"
            >
              {status === "uploading" ? (
