...
export function VideoPlayer({ episode, initialTimeSec = 0, onProgress, onEnded }: Props) {
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      if (initialTimeSec > 0 && video.duration > initialTimeSec) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-[0.16em] text-zinc-400">Now playing</div>
          <div className="text-lg font-semibold text-white">
            Episode {episode.index + 1}: {episode.title}
          </div>
        </div>
